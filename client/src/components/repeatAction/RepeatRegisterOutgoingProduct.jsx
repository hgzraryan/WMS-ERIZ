...
console.log(outgoingProduct)
  ...
  // };
  useEffect(() => {
    //setProductClassType(outgoingProduct.currentProductId)
    if (!outgoingProduct?.currentProductId) return;

    const fetchData = async () => {
        const tmp = await fetchDataByProduct(outgoingProduct?.currentProductId);
        setFetchedProductsList(tmp);
    };

    fetchData();
}, [outgoingProduct?.currentProductId]);
                                  ...
                                  <div className="form-control d-flex justify-content-between">
                                    <div className="flex-grow-1 me-1">
                                      ...
                                      <Controller
                                        ...
                                        defaultValue={
                                            outgoingProduct
                                                ? { productListId: outgoingProduct?.currentProductId, label: outgoingProduct?.name }
                                                : null
                                        }
                                        rules={{ required: true }}
                                        render={({ field }) => (
...
export default RepeatRegisterOutgoingProduct;
