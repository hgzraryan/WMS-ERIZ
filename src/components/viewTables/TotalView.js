...
function TotalView({data,column,dataReceived}) {
  ...
  return (
    <>    
    <table className="table nowrap mb-5 dataTable no-footer total-view-table p-1 " style={{ }}{...getTableProps()}>
    <thead >
      {headerGroups.map((headerGroup) => (
        <tr {...headerGroup.getHeaderGroupProps()}>
          {headerGroup.headers.map((column) => (
            <th  {...column.getHeaderProps(column.getSortByToggleProps())} style={{fontSize:'12px',backgroundColor:'#018a54', height:'15px', display:'flex', alignItems:'center'}}>
                  {column.id !== "selection" && (
      ...
      {data?.length > 0 ? (
        <tbody {...getTableBodyProps()} >
          {rows.map(row => {
            ...
            return (
              <tr {...row.getRowProps({
                style: { width: '100%',fontSize:'12px',height:'20px',display:'flex', alignItems:'center' },
